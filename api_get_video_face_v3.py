...
def api(P1,P2,P3):
	...
	try:
		...
		while(frame_count<int(P3)):
			...
			if frame_count%30==0: first_frame=frame #background 每 30 frames refresh一次
			frame_count += 1
			...
			#------- 去除背景
			cut_frame = cv2.resize(frame,(pw,ph),interpolation=cv2.INTER_AREA)
			gframe,cframe=pd.FilterFrame(cut_frame,first_frame,pw,ph)
			#-------		
			sframe,imgs, bboxes = DetBodyFaces(cframe,cframe)	#dnn 作法
