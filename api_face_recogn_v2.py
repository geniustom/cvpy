...
Note: 
	置信度>0.6 : 
		直接輸出
	置信度在 0.4~6 之間 :
		若置信度>平均值+1.5個標準差 : 直接輸出
		否則第一名必須比第二名高出0.5個標準差
	置信度或平均值<0.4 : 無效輸出
'''

##################### 可變動參數區 #####################
fix_wh=200
confidence=0.6 #0~1的置信度 , 越高越嚴格, 0.5以上才有參考意義, 0.6以上準確度較高
threshold=0.5
untrust=0.4
########################################################
...
P2_DEFAULT="unit_test_img/negtive/SWB000mgLbUm-1566541802.jpg"
...
def get_face_profile_single_person(face_names,face_vectors,face_id_lists,test_face_imgs,test_locations,test_encodings):
	result_person=[] #{}
	result_locat=[]
	fid=""
	...
	if max_conf<confidence: 
		if max_conf<untrust: name="?"
		elif max_conf>1.5*np.std(conf)+np.mean(conf) and max_conf>threshold : pass
		elif conf_sort[0]-conf_sort[1]< 0.5*np.std(conf): name="?"
...
def api(P1,P2,P3):
	...
	try:
		...
		if os.path.isfile(P2):
			...
			if len(test_encodings)>0:
				detected_person,max_conf,max_conf_name=get_face_profile_single_person(face_names,face_vectors,face_id_lists,test_face_imgs,test_locations,test_encodings)
	...
	log.Json_log(l,"most_like_confidence",max_conf)
	log.Json_log(l,"most_like_person",max_conf_name)
